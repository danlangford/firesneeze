namespace GooglePlayGames.Native.PInvoke
{
    ...
    internal class RealtimeManager
    {
        ...
        internal void SendUnreliableMessageToSpecificParticipants(NativeRealTimeRoom room, List<GooglePlayGames.Native.PInvoke.MultiplayerParticipant> recipients, byte[] data)
        {
            ...
            {
                <>f__am$cache1 = r => r.AsPointer();
            }
            RealTimeMultiplayerManager.RealTimeMultiplayerManager_SendUnreliableMessage(this.mGameServices.AsHandle(), room.AsPointer(), recipients.Select<GooglePlayGames.Native.PInvoke.MultiplayerParticipant, IntPtr>(<>f__am$cache1).ToArray<IntPtr>(), new UIntPtr((ulong) recipients.LongCount<GooglePlayGames.Native.PInvoke.MultiplayerParticipant>()), data, PInvokeUtilities.ArrayToSizeT<byte>(data));
        }
        ...
        private static IntPtr ToCallbackPointer(Action<RealTimeRoomResponse> callback) => 
            Callbacks.ToIntPtr<RealTimeRoomResponse>(callback, new Func<IntPtr, RealTimeRoomResponse>(RealTimeRoomResponse.FromPointer));
