namespace GooglePlayGames.Native.PInvoke
{
    ...
    internal class RealTimeEventListenerHelper : BaseReferenceHolder
    {
        ...
        {
            <ToCallbackPointer>c__AnonStorey11D storeyd = new <ToCallbackPointer>c__AnonStorey11D {
                callback = callback
            };
            Action<IntPtr> action = new Action<IntPtr>(storeyd.<>m__DE);
            return Callbacks.ToIntPtr(action);
        }

        [CompilerGenerated]
        private sealed class <ToCallbackPointer>c__AnonStorey11D
        {
            internal Action<NativeRealTimeRoom> callback;

            internal void <>m__DE(IntPtr result)
            {
