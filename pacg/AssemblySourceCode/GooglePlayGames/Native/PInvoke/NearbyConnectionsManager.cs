namespace GooglePlayGames.Native.PInvoke
{
    ...
    internal class NearbyConnectionsManager : BaseReferenceHolder
    {
        ...
        internal string LocalDeviceId() => 
            PInvokeUtilities.OutParamsToString((out_arg, out_size) => NearbyConnections.NearbyConnections_GetLocalDeviceId(base.SelfPtr(), out_arg, out_size));

        internal string LocalEndpointId() => 
            PInvokeUtilities.OutParamsToString((out_arg, out_size) => NearbyConnections.NearbyConnections_GetLocalEndpointId(base.SelfPtr(), out_arg, out_size));
        ...
        internal void StartAdvertising(string name, List<NativeAppIdentifier> appIds, long advertisingDuration, Action<long, NativeStartAdvertisingResult> advertisingCallback, Action<long, NativeConnectionRequest> connectionRequestCallback)
        {
            ...
            {
                <>f__am$cache1 = id => id.AsPointer();
            }
            NearbyConnections.NearbyConnections_StartAdvertising(base.SelfPtr(), name, appIds.Select<NativeAppIdentifier, IntPtr>(<>f__am$cache1).ToArray<IntPtr>(), new UIntPtr((ulong) appIds.Count), advertisingDuration, new NearbyConnectionTypes.StartAdvertisingCallback(NearbyConnectionsManager.InternalStartAdvertisingCallback), Callbacks.ToIntPtr<long, NativeStartAdvertisingResult>(advertisingCallback, new Func<IntPtr, NativeStartAdvertisingResult>(NativeStartAdvertisingResult.FromPointer)), new NearbyConnectionTypes.ConnectionRequestCallback(NearbyConnectionsManager.InternalConnectionRequestCallback), Callbacks.ToIntPtr<long, NativeConnectionRequest>(connectionRequestCallback, new Func<IntPtr, NativeConnectionRequest>(NativeConnectionRequest.FromPointer)));
        }
